...
function isAdminUser(){
  return currentUser && String(currentUser.role || "").trim().toUpperCase() === "ADMIN";
}
...
function handleRouteState(state){
  ...
  if(route === "unitInfo") return isAdminUser() ? openUnitInfo(data.unit || "", false) : showDashboard(false);
  if(route === "checkForm") return openCheckForm(data.unit || "", data.base || "", false);
...
function resetLoginScreen(){
  ...
  const loginBtn = document.getElementById("loginBtn") || document.querySelector('#loginView button');
...
function renderAdminViewSwitch(){
  ...
  const isAdmin = currentUser && String(currentUser.role || "").trim().toUpperCase() === "ADMIN";
...
function loadDashboardApparatus(){
  ...
  const userBase =
    String(currentUser.base || "").trim().toUpperCase();

  const showAllChecksheets =
    isAdmin && adminDashboardMode === "all";

  const grid = document.getElementById("apparatusGrid");

  grid.innerHTML =
    `<p style="text-align:center;color:#60a5fa;grid-column:1/-1;">Loading apparatus...</p>`;
  ...
  if(showAllChecksheets || userBase === "BOTH"){
    ...
    url = API_URL + "/api/apparatus?base=" + encodeURIComponent(currentUser.base);
  }
  ...
  fetch(url)
    ...
    .catch(error => {
      grid.innerHTML =
        `<div class="admin-row" style="grid-column:1/-1;text-align:center;">${escapeHtml(error.message)}</div>`;
    });
}
...
function showApparatus(units){
  ...
  units.forEach(entry => {
    ...
    const checklistBase = (entry && typeof entry === "object")
      ? deriveChecklistBaseFromUnit(unit, entry.checklistBase || entry.homeBase || entry.base || currentUser.base)
      : deriveChecklistBaseFromUnit(unit, currentUser.base);

    const currentBase = (entry && typeof entry === "object")
      ? (entry.currentBase || checklistBase)
      : checklistBase;

    const isAdminAllCheckSheets = currentUser
      && String(currentUser.role || "").trim().toUpperCase() === "ADMIN"
      && adminDashboardMode === "all";

    const checkedToday = entry && typeof entry === "object" && entry.checkedToday === true;
...
function openCheckForm(unit, baseOrAddToHistory=true, addToHistory=true){
  ...
  if(typeof baseOrAddToHistory === "boolean"){
    ...
    base = currentCheckBase || (currentUser && currentUser.base ? currentUser.base : "");
  } else {
    base = String(baseOrAddToHistory || "").trim();
  }

  if(!base || base.toUpperCase() === "BOTH"){
    base = currentUser && currentUser.base && String(currentUser.base).toUpperCase() !== "BOTH"
      ? currentUser.base
      : "";
  }
  ...
  const todayUrl = API_URL + "/api/check-submissions?unit=" + encodeURIComponent(unit) + "&base=" + encodeURIComponent(base);
  ...
  fetch(todayUrl)
    ...
    .then(result => {
      ...
      const checklistUrl = API_URL + "/api/checklist?unit=" + encodeURIComponent(unit) + "&base=" + encodeURIComponent(base);
...
function hasChecklistType(typeValue, wanted){
  return parseChecklistTypes(typeValue).includes(String(wanted || "").toUpperCase());
}
...
function getSelectedChecklistTypes(){
  const checked = Array.from(document.querySelectorAll(".builderTypeOption:checked"))
    .map(box => box.value);
...
function getSelectedBuilderUnitInfo(){
  ...
  if(value.includes("||")){
    ...
    return { base: deriveChecklistBaseFromUnit(unit, parts[0] || ""), unit: unit };
  }
...
function getCheckDraftKey(unit){
  const base = deriveChecklistBaseFromUnit(unit, currentCheckBase || (currentUser && currentUser.base ? currentUser.base : ""));
  const today = new Date().toLocaleDateString("en-US");
  return "apparatusDraft_" + String(base || "BASE") + "_" + String(unit || "UNIT").replace(/\s+/g,"_") + "_" + today;
}
...
function getCheckDraftData(){
  ...
  return cards.map(card => ({
    ...
    bagChecks: Array.from(card.querySelectorAll(".bagSubCheck")).map(cb => !!cb.checked)
  }));
}
...
function saveCheckDraft(unit){
  try{
    ...
    const payload = {
      ...
      unit: unit,
      base: deriveChecklistBaseFromUnit(unit, currentCheckBase || (currentUser && currentUser.base ? currentUser.base : "")),
...
function restoreCheckDraft(unit){
  try{
    ...
    cards.forEach((card,index) => {
      ...
      if(card.querySelector(".onApparatus")) card.querySelector(".onApparatus").value = row.onApparatus || "";
      if(card.querySelector(".functional")) card.querySelector(".functional").value = row.functional || "";
      if(card.querySelector(".yesNo")) card.querySelector(".yesNo").value = row.yesNo || "";
      if(card.querySelector(".numberValue")) card.querySelector(".numberValue").value = row.numberValue || "";
      if(card.querySelector(".percentageValue")) card.querySelector(".percentageValue").value = row.percentageValue || "";
      if(card.querySelector(".expDateValue")) card.querySelector(".expDateValue").value = row.expDateValue || "";
      if(card.querySelector(".expDate2Value")) card.querySelector(".expDate2Value").value = row.expDate2Value || "";
      if(card.querySelector(".fuelValue")) card.querySelector(".fuelValue").value = row.fuelValue || "";
      if(card.querySelector(".oilValue")) card.querySelector(".oilValue").value = row.oilValue || "";
      if(card.querySelector(".psiValue")) card.querySelector(".psiValue").value = row.psiValue || "";
      if(card.querySelector(".psi2Value")) card.querySelector(".psi2Value").value = row.psi2Value || "";
      if(card.querySelector(".seal1Value")) card.querySelector(".seal1Value").value = row.seal1Value || "";
      if(card.querySelector(".seal2Value")) card.querySelector(".seal2Value").value = row.seal2Value || "";
      if(card.querySelector(".testNumberValue")) card.querySelector(".testNumberValue").value = row.testNumberValue || "";
      if(card.querySelector(".textValue")) card.querySelector(".textValue").value = row.textValue || "";
      if(card.querySelector(".notes")) card.querySelector(".notes").value = row.notes || "";
...
function buildCheckForm(unit, items){
  ...
  pages.forEach((page, pageIndex) => {
    ...
    page.items.forEach(item => {
      ...
      const bagSubitems = parseBagSubitems(item.subitems || item.bagItems || "");
  ...
  const firstPageNav = document.querySelector("#checkForm .section-page.active .page-nav");
  if(firstPageNav && !document.getElementById("clearSavedProgressBtn")){
    firstPageNav.insertAdjacentHTML("beforebegin", `
...
function getItemReviewAnswer(card){
  ...
  if(typeList.includes("BAG") || bagRows.length > 0){
    lines.push(`<div class="review-line"><strong>Inside Item Checklist:</strong></div>`);
    bagRows.forEach(row => {
      const subItem = row.dataset.bagSubitem || row.querySelector(".bagSubCheck")?.dataset.subitem || "Item";
...
function submitCheck(unit){
  const submitButton = document.getElementById("saveCheckBtn") || document.querySelector(".check-actions button");
  ...
  cards.forEach(card => {
    ...
    if(typeList.includes("PERCENTAGE")){
      ...
      if(value !== ""){
        ...
        if(isNaN(percent) || percent < 0 || percent > 100){
          missing.push(item + " - Battery percentage must be between 0 and 100");
        }
      }
    ...
    if(typeList.includes("SEAL1")){
      const seal1 = String(card.querySelector(".seal1Value")?.value || "").trim();
    ...
    if(typeList.includes("SEAL2")){
      const seal2 = String(card.querySelector(".seal2Value")?.value || "").trim();
    ...
    if(typeList.includes("TESTNUMBER")){
      const testNumber = String(card.querySelector(".testNumberValue")?.value || "").trim();
    ...
    if(typeList.includes("BAG") || bagRows.length > 0){
      bagRows.forEach(row => {
        const subItem = row.dataset.bagSubitem || row.querySelector(".bagSubCheck")?.dataset.subitem || "Bag Item";
        const checked = !!row.querySelector(".bagSubCheck")?.checked;
        const answer = checked ? "Checked" : "Not Checked";
      ...
      const bagNotes = String(card.querySelector(".bagNotes")?.value || "").trim();
      if(bagNotes){
    ...
    items.push({section,subsection,shelf,item,type,value,status,notes});
  });
  ...
  const checkData = {
    ...
    signature: signature,
    signatureName: document.getElementById("signatureName") ? document.getElementById("signatureName").value : currentUser.name,
    items: items
  };
...
function submitMoveUnit(){
  ...
  const unitId =
    document.getElementById("moveUnit").value;

  const newBase =
    document.getElementById("moveBase").value;
...
function addNewApparatus(){
  ...
  const checkDays = document.getElementById("newUnitCheckDays").value.trim() || "DAILY";
...
function clearChecklistBuilderForm(keepUnit=true){
  ...
  if(document.getElementById("builderSection")) // Section kept after add so you can keep adding to same compartment/shelf
  // Subsection kept after add
  // Shelf kept after add
  if(document.getElementById("builderItem")) document.getElementById("builderItem").value = "";
  if(document.getElementById("builderQty")) document.getElementById("builderQty").value = "";
  if(document.getElementById("builderSubitems")) document.getElementById("builderSubitems").value = "";
  if(document.getElementById("hasShelf")) document.getElementById("hasShelf").checked = false;
  document.querySelectorAll(".builderTypeOption").forEach(box => box.checked = false);

  if(keepUnit && currentUnitInfo.unit && document.getElementById("builderUnit")){
...
function editChecklistBuilderItem(id){
  const item = checklistBuilderItemsCache.find(i => String(i._id) === String(id));

  if(!item){
    alert("Could not find this item. Reload the checklist builder and try again.");
    return;
  }
  ...
  document.getElementById("checklistBuilderFormTitle")
    .scrollIntoView({ behavior: "smooth", block: "start" });
}

function saveChecklistBuilderItem(){
  const editRow = document.getElementById("builderEditRow") ? document.getElementById("builderEditRow").value : "";
  ...
  const subsection = document.getElementById("builderSubsection") ? document.getElementById("builderSubsection").value.trim() : "";
  const shelf = document.getElementById("builderShelf") ? document.getElementById("builderShelf").value.trim() : "";
  ...
  const subitems = document.getElementById("builderSubitems") ? document.getElementById("builderSubitems").value.trim() : "";
  ...
  fetch(API_URL + "/api/checklist", {
    ...
    .then(result => {
      ...
      showToast(editRow ? "Checklist item updated." : "Checklist item added.", "success");
...
function saveChecklistOrder(){
  ...
  const ids = Array.from(document.querySelectorAll("#builderReorderList .builder-reorder-row"))
...
function formatRecentDateInput(input){
  let value = String(input.value || "").replace(/\D/g, "").substring(0,8);
...
function renderRecentChecksList(checks, selectedDate){
  ...
  if(selectedDate){
    filtered = checks.filter(c => String(c.date || "").trim() === selectedDate);
  }
...
function getValuePart(value, label){
  ...
  const match = parts.find(p => p.toLowerCase().startsWith(label.toLowerCase() + ":"));
  if(!match) return "";
...
function formatDetailAnswer(d){
  ...
  if(type === "ONTRUCK"){
    lines.push(`<div class="review-line"><strong>On Truck:</strong> ${escapeHtml(getValuePart(value, "On Apparatus") || value)}</div>`);
  }
  else if(type === "FUNCTIONAL"){
    lines.push(`<div class="review-line"><strong>Functional:</strong> ${escapeHtml(getValuePart(value, "Functional") || value)}</div>`);
  }
  else if(type === "PASSFAIL"){
    lines.push(`<div class="review-line"><strong>On Truck:</strong> ${escapeHtml(getValuePart(value, "On Apparatus"))}</div>`);
    lines.push(`<div class="review-line"><strong>Functional:</strong> ${escapeHtml(getValuePart(value, "Functional"))}</div>`);
  }
  else if(type === "FUEL"){
    lines.push(`<div class="review-line"><strong>Fuel:</strong> ${escapeHtml(value)}</div>`);
  }
  else if(type === "DATE"){
    lines.push(`<div class="review-line"><strong>Exp:</strong> ${escapeHtml(value)}</div>`);
  }
  else if(type === "NUMBER"){
    lines.push(`<div class="review-line"><strong>Number:</strong> ${escapeHtml(value)}</div>`);
  }
  else {
    lines.push(`<div class="review-line"><strong>Text:</strong> ${escapeHtml(value)}</div>`);
  }

  lines.push(`<div class="review-line"><strong>Notes:</strong> ${escapeHtml(d.notes || "")}</div>`);
  return lines.join("");
}
...
function logout(){
  ...
  showLogin(false);
  history.replaceState({ route: "login", data: {} }, "", location.pathname + location.search);
}
...
function loadMessageBoard(messages){
  ...
  messages.forEach(msg => {
    ...
    const priority = String(msg.priority || "").trim().toUpperCase();
...
function renderFleetMap(units){
  ...
  units.forEach(u => {
    const base = normalizeBaseLabel(u.currentBase || u.base || u.homeBase || "Unknown");
    if(!grouped[base]) grouped[base] = [];
    grouped[base].push(u);
  });

  let bases = Object.keys(grouped).sort((a,b) => a.localeCompare(b, undefined, {numeric:true}));
  ...
  if(units.length === 0){
    ...
    bases.forEach(base => {
      html += `
        <div class="fleet-base-card" data-search="${escapeHtml(base.toLowerCase())} ${escapeHtml(grouped[base].map(u => u.unit).join(' ').toLowerCase())}">
...
function filterFleetMapCards(){
  const q = String(document.getElementById("fleetMapSearch")?.value || "").toLowerCase().trim();
  document.querySelectorAll("#fleetMapCards .fleet-base-card").forEach(card => {
...
function fleetInfoCardHtml(u, clickable){
  ...
  const search = JSON.stringify(Object.values(u || {}).join(" ").toLowerCase()).slice(1,-1);
...
function filterFleetInfoCards(){
  const q = String(document.getElementById("fleetInfoSearch")?.value || "").toLowerCase().trim();
  document.querySelectorAll("#fleetInfoCards .fleet-info-card").forEach(card => {
    card.style.display = !q || String(card.dataset.search || "").includes(q) ? "" : "none";
  });
}
